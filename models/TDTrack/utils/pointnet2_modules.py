...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz, features, npoint, return_idx=False):
        ...
        if self.use_fps:
            sample_idxs = pointnet2_utils.furthest_point_sample(
                xyz, self.npoint)
        else:
            sample_idxs = torch.arange(self.npoint).repeat(
...
class FlowEmbedding(nn.Module):
    ...
    def forward(self, xyz1, xyz2, feature1, feature2):
        ...
        B, N, C = xyz1.shape
        if self.knn:
            idx = pointnet2_utils.knn_point(
                self.nsample, xyz1, xyz2)  # (B, npoint, nsample)
        else:
            idx, cnt = pointnet2_utils.ball_query(
                self.radius, self.nsample, xyz2, xyz1)  # (B, npoint, nsample)

        xyz2_grouped = pointnet2_utils.grouping_operation(
            xyz2_t, idx)  # (B, 3, npoint, nsample)
        # (B, 3, npoint, nsample)
        pos_diff = xyz2_grouped - xyz1_t.view(B, -1, N, 1)

        feat2_grouped = pointnet2_utils.grouping_operation(
            feature2, idx)  # [B, C, npoint, nsample]
        if self.corr_func == 'concat':
            feat_diff = torch.cat([feat2_grouped, feature1.view(
                B, -1, N, 1).repeat(1, 1, 1, self.nsample)], dim=1)

        # [B, 2*C+3,npoint, nsample]
...
class PointNetSetUpConv(nn.Module):
    ...
    def forward(self, xyz1, xyz2, feature1, feature2):
        ...
        if self.knn:
            idx = pointnet2_utils.knn_point(
                self.nsample, xyz1, xyz2)  # (B, npoint1, nsample)
        else:
            idx, cnt = pointnet2_utils.ball_query(
                self.radius, self.nsample, xyz2, xyz1)  # (B, npoint1, nsample)
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        ...
        new_features.backward(torch.cuda.FloatTensor(
            *new_features.size()).fill_(1))
        print(new_features)
        print(xyz.grad)
